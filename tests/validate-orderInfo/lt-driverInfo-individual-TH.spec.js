...
test.describe('Validate form on Order Info page', () => {
    ...
    /*     test('driver info: select manual input', async ({ page }) => {
            const trigger = page.locator('#driver-info-accordion-trigger-id');
            const content = page.locator('#radix-_r_6_');
    
            // เปิด accordion ให้แน่ใจ
            await trigger.scrollIntoViewIfNeeded();
    ...
    test('driver id card: should show required error when empty', async ({ page }) => {
        const idInput = page.locator('#driver-id-card-input-id');
        ...
        await expect(
            page.getByText('กรุณากรอกเลขบัตรประชาชน').first()
        ).toBeVisible();
        await page.waitForTimeout(1000);
    });

    test('driver id card: empty validation', async ({ page }) => {
        ...
        for (const message of manualErrors) {
            await expect(
    ...
    test('should invalidate id card and allow re-input', async ({ page }) => {
        ...
        for (const idcard of invalidIDcard) {
            // กรอกเลขบัตรผิด
            await idInput.pressSequentially(idcard, { delay: 80 });
            await idInput.blur();
            await nextButton.click();
    ...
    test('should accept valid id card and allow re-input', async ({ page }) => {
        ...
        const idCardInput = page.locator('#driver-id-card-input-id');
        const nextButton = page.getByRole('button', { name: /บันทึก|ต่อไป/ });
        ...
        for (const idcard of validIDcard) {
            ...
            // trigger validation
            await nextButton.click();
            await expect(idCardInput).not.toHaveAttribute('aria-invalid', 'true'); // field ต้อง NOT invalid
