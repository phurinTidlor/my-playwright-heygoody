...
test('heygoody longterm e2e Ev bymyself flow without CMI (no พ.ร.บ.)', async ({ page }) => {
    test.setTimeout(180_000);
    await page.goto(baseURL);
    await page.waitForLoadState('networkidle');
    ...
    await openAccordionByText(page, 'ข้อมูลรถ');
    await humanFillText(page.locator('#license-plate-id'), '2เฮ้2026');
    await page.waitForTimeout(500);
    await humanFillText(page.locator('#chassis-number-id'), 'MRH123456789ABCDE');
    await humanFillText(page.locator('#engine-number-id'), 'ENGINENUMBEREVTES');
    await page.waitForTimeout(500);
